...
export function doraSummaryJob(profile: StackProfile): NormalJob {
  const job = new NormalJob('dora-summary', {
  ...
  job.addStep(
    new Step({
      ...
      env: {
        PIPELINE_STATUS:
          "${{ contains(needs.*.result, 'failure') && 'failure' || (contains(needs.*.result, 'cancelled') && 'cancelled' || 'success') }}",
        ...
        SUBJECT:
          '${{ github.event.pull_request.title || github.head_ref || github.ref_name }}',
      },
      run: dedent([
